...
class GetBadMachinery
  ...
  def prep_chapters
    ...
    page = agent.get("http://scarygoround.com/ar.php")
    @chapter_markers = {}
    i = 1
    @first_page = 0
    load_chapter = 1
    ...
    page.search("//a[contains(@href,'?date=')]").each{ |anchor|
      marker = anchor.attributes["href"].to_s.split('?date=')[-1].to_s
      unless marker == "archive"
  ...
  def start_download
    ...
    page = @agent.get("http://scarygoround.com/?date=#{@first_page}")
    ...
    while keep_going
      ...
      current_page_id = page.uri().to_s.split("http://scarygoround.com/?date=")[-1]
      ...
      # binding.pry
      comic_image = page.search("//img[contains(@src,'strips')]").first.attributes["src"].to_s
      comic_url = "http://scarygoround.com/#{comic_image}"
  ...
  def chapter_directory(chapter_id)
    "badmachinery/chapter_#{chapter_id}"
  end
...
puts "What chapter range of Bad Machinery would you like to download?"
...
if ['yes','Yes','y','Y'].any?{ |word| response.include?(word) }
  GetBadMachinery.new_download(chapter_range_array)
else
  puts "Exiting..."
end
